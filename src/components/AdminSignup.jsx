  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/admin/sign-up`,
        formData,
      );
      ...
      if (response.data.success) {
        ...
        navigate(`/verify-admin/${response.data.id}`)
      }
...
<p className="text-white mb-5 text-[14px] sm:text-md">Already have an account <Link to={'/admin-login'} className="text-blue-400 underline">Login</Link></p>
        {loading ? (
